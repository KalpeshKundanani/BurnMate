...
A Context Capsule is a structured block of metadata that **must** be included at the start of every AI invocation within this framework. It anchors the invocation to the current task, prevents hallucination of scope, and enables deterministic resumability.
...
| `role` | Yes | The role this invocation is operating as. Must match `ROLES.md`. |
| `relevant_documents` | Yes | List of file paths the invocation must read before acting. Minimum one document. |
| `objective` | Yes | A single sentence describing the goal of this invocation. No ambiguity. |
| `constraints` | Yes | Boundaries the invocation must not cross. At minimum: role boundaries and state boundaries. |
...
2. **The capsule is the single source of scope.** The invocation must not act outside the objective, constraints, and expected output defined in the capsule.
3. **State must be verified before acting.** The invocation must confirm that `current_state` matches the actual persisted state of the slice. If there is a mismatch, the invocation must halt and report the discrepancy.
4. **Role must be verified before acting.** The invocation must only perform actions allowed by the role specified in the capsule, as defined in `ROLES.md`.
5. **Relevant documents must be read.** The invocation must read all documents listed in `relevant_documents` before producing output.
6. **Output must match the expected format and artifact path.** The invocation produces exactly what the capsule specifies, nothing more.
...
Without a capsule, an invocation will:
...
The capsule eliminates these failure modes by constraining every invocation to a well-defined box. The role execution knows what role it is operating in, what state it's in, what it must read, what it must do, and what it must produce. Nothing else.
